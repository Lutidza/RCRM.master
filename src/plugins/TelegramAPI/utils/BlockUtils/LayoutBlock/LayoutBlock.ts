...
export async function sendLayoutBlock(
  ...
  try {
    ...
    if (layoutBlock.clearPreviousMessages && ctx.chat) {
      log('debug', `Очищаем предыдущие сообщения перед LayoutBlock "${layoutBlock.name}"`);
    ...
    log('error', `Ошибка при обработке лейаута "${layoutAlias}": ${(error as Error).message}`, payload);
